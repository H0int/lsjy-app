...
function renderDashboard() {
  ...
  // 统计卡片
  html += statCard('fa-users', '用户总数', stats.totalUsers, '+12%', 'var(--p)');
  html += statCard('fa-user-plus', '今日新增', stats.todayNewUsers, '+3', 'var(--p2)');
  html += statCard('fa-bolt', '算力总量', stats.totalCredits, '今日+'+stats.todayCredits, '#f59e0b');
  ...
  html += statCard('fa-clock', '待审批', pendingCount, '需处理', pendingCount > 0 ? 'var(--err)' : 'var(--ok)');
...
function renderUsers() {
  ...
  html += '<div class="toolbar-left"><input type="text" id="user-search" placeholder="搜索用户名..." oninput="AdminAPI.filterUsers()">';
  html += '<select id="user-filter" onchange="AdminAPI.filterUsers()"><option value="">全部状态</option><option value="pending">待审批</option><option value="approved">已通过</option><option value="banned">已封禁</option><option value="rejected">已拒绝</option></select></div>';
  html += '<div class="toolbar-right"><button class="admin-btn" onclick="AdminAPI.batchApprove()"><i class="fa-solid fa-check-double"></i> 批量通过</button></div>';
  html += '</div>';
...
function renderTools() {
  ...
  var html = '<div class="page-toolbar">';
  html += '<div class="toolbar-left"><span style="color:var(--nd);">共 <strong style="color:var(--p)">'+totalTools+'</strong> 个工具，已启用 <strong style="color:var(--ok)">'+enabledTools+'</strong> 个</span></div>';
  html += '<div class="toolbar-right"><button class="admin-btn" onclick="AdminAPI.resetTools()"><i class="fa-solid fa-rotate"></i> 重置默认</button></div>';
  html += '</div>';
...
function renderFinance() {
  ...
  html += '<div class="tab-item" onclick="AdminAPI.switchFinanceTab(\'payments\',this)">支付方式</div></div>';
...
function renderContent() {
  ...
  html += '<div class="tab-item" onclick="AdminAPI.switchContentTab(\'changelog\',this)">版本日志</div></div>';
...
function renderContentAnnouncements() {
  ...
  anns.forEach(function(a) {
    ...
    html += '<td><button class="btn-xs" onclick="AdminAPI.editAnnouncement('+a.id+')"><i class="fa-solid fa-pen"></i></button>';
...
function renderContentChangelog() {
  ...
  logs.forEach(function(l) {
    html += '<div style="margin-bottom:20px;padding-left:20px;border-left:3px solid var(--p);">';
    html += '<div style="font-size:16px;font-weight:700;color:var(--p);">'+l.version+' <span style="font-size:12px;color:var(--nd);font-weight:400;">'+l.date+'</span></div>';
...
function renderPermissions() {
  ...
  html += '<div class="tab-item" onclick="AdminAPI.switchPermTab(\'admins\',this)">管理员账号</div></div>';
