<template>
  <div>
    <div class="cyber-toolbar">
      <div class="tab-bar">
        <button v-for="range in ranges" :key="range.value" @click="switchRange(range.value)" class="cyber-tab" :class="{ 'cyber-tab-active': activeRange === range.value }">{{ range.label }}</button>
      </div>
      <div class="flex-1" />
...
async function fetchData() {
  const res = await adminApi.getDataReports(activeRange.value)
