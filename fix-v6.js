// lsjy-app fix v6 - Complete interaction layer for CDN bypass
(function(){
"use strict";

// === 1. Credits System ===
function initCredits(){
    var r=localStorage.getItem('lsjy3_credits');var c;
    try{c=JSON.parse(r)}catch(e){c=null}
    if(typeof c!=='object'||c===null){
        c={'KF02V9':10000};
        try{var u=JSON.parse(localStorage.getItem('lsjy3_users')||'[]');for(var i=0;i<u.length;i++){if(!c[u[i].username])c[u[i].username]=100}}catch(e){}
        localStorage.setItem('lsjy3_credits',JSON.stringify(c));
    } else {
        if(!c['KF02V9']){c['KF02V9']=10000;localStorage.setItem('lsjy3_credits',JSON.stringify(c))}
    }
}
initCredits();

window.getCredits=function(){try{return JSON.parse(localStorage.getItem('lsjy3_credits'))||{}}catch(e){return{}}};
window.getUserCredits=function(){var u=getCur();if(!u)return 0;return getCredits()[u.username]||0};
...
window.updateCreditsUI=function(){
    ...
    if(el)el.textContent=getUserCredits();
    var cr=document.getElementById('navCredits');
    if(cr){cr.style.display=getCur()?'inline-flex':'none';}
};

// === 2. Create tool overlay if missing ===
function ensureToolOverlay(){
    ...
    ov.innerHTML='<div id="toolPanel" style="background:#fff;border-radius:16px;max-width:600px;width:90%;max-height:85vh;overflow-y:auto;padding:24px;position:relative;"></div><button id="closeToolBtn" style="position:absolute;top:12px;right:16px;font-size:24px;background:none;border:none;cursor:pointer;color:#666;z-index:10;">&times;</button>';
    document.body.appendChild(ov);
    document.getElementById('closeToolBtn').onclick=function(){closeTool()};
    ov.addEventListener('click',function(e){if(e.target===ov)closeTool()});
}
...
// === 3. Create auth modal if missing ===
function ensureAuthModal(){
    ...
    am.innerHTML='<div style="background:#fff;border-radius:16px;padding:32px;max-width:420px;width:90%;position:relative;"><button onclick="document.getElementById(\'authModal\').style.display=\'none\'" style="position:absolute;top:12px;right:16px;font-size:24px;background:none;border:none;cursor:pointer;color:#666;">&times;</button><div id="authContent"></div></div>';
...
window.openAuth=function(mode){
    ...
    if(!am||!ac){ensureAuthModal();am=document.getElementById('authModal');ac=document.getElementById('authContent');}
    if(!am||!ac)return;
    ...
    +'<button id="authSubmitBtn" style="width:100%;padding:12px;background:linear-gradient(135deg,#e53e3e,#c53030);color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;">'+(isLogin?'登录':'注册')+'</button>'
    +'<p style="text-align:center;margin-top:12px;font-size:13px;color:#888;">'+(isLogin?'没有账号？<a href="javascript:void(0)" id="authSwitchLink">立即注册</a>':'已有账号？<a href="javascript:void(0)" id="authSwitchLink">立即登录</a>')+'</p>';
    document.getElementById('authSubmitBtn').onclick=function(){doAuth(mode)};
    document.getElementById('authSwitchLink').onclick=function(){openAuth(isLogin?'register':'login')};
...
window.updateNavUI=function(){
    var cur=getCur();
    var lr=document.getElementById('navLoginBtn');
    var nr=document.getElementById('navRight');
    var cr=document.getElementById('navCredits');
    if(cur){
        if(lr){lr.textContent=cur.username;lr.onclick=function(){doLogout()};lr.removeAttribute('data-target');}
        if(!cr&&nr){
            ...
            document.getElementById('creditDisplay').textContent=getUserCredits();
...
window.filterToolCat=function(cat){
    ...
    var btns=document.querySelectorAll('#toolCats button, .tool-cat-btn');
...
window.openTool._v6=true;

// === 9. renderGenericAIText ===
window.renderGenericAIText=function(toolId){
    ...
    (t.fields||[]).forEach(function(f){
        h+='<div style="margin-bottom:14px;"><label style="display:block;font-size:13px;color:#555;margin-bottom:4px;font-weight:500;">'+f.label+'</label>';
        if(f.type==='select'){
    ...
    h+='<button id="aiGenBtn" style="width:100%;padding:12px;background:linear-gradient(135deg,#e53e3e,#c53030);color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;margin-top:8px;">🚀 AI 生成</button>';
    h+='<div id="aiResult" style="margin-top:16px;display:none;"><div id="aiResultText" style="background:#f8f9fa;border-radius:8px;padding:16px;white-space:pre-wrap;font-size:14px;line-height:1.7;color:#333;max-height:400px;overflow-y:auto;"></div><button id="copyResultBtn" style="margin-top:8px;padding:8px 16px;background:#4299e1;color:#fff;border:none;border-radius:6px;font-size:13px;cursor:pointer;">📋 复制内容</button></div>';
    h+='</div>';
    return h;
};

// === 10. runAIText - Doubao API ===
window.runAIText=function(toolId){
    ...
    var messages=[{role:'system',content:t.prompt||'你是AI助手。'},{role:'user',content:input}];
...
function cleanAdmin(){
    ...
    for(var i=0;i<links.length;i++){
        ...
        if(href==='admin/'||href==='/admin/'||href==='admin'){
            ...
            if(t==='管理后台'||t==='后台管理'||t==='管理')links[i].style.display='none';
        }
    }
}

// === 13. Bind ALL events (critical!) ===
function bindEvents(){
    // Bind search input - try multiple selectors
    var si=document.querySelector('input[placeholder*="搜索"]');
    if(!si)si=document.getElementById('searchInput');
    if(!si)si=document.querySelector('input[type="text"][class*="search"]');
    if(si&&!si._v6bound){
        si.addEventListener('input',function(){searchTools(this.value)});
        si._v6bound=true;
    }

    // Bind category buttons
    var catBtns=document.querySelectorAll('#toolCats button, .tool-cat-btn, [data-cat-btn]');
    catBtns.forEach(function(b){
        if(!b._v6bound){
            b.addEventListener('click',function(e){
                e.preventDefault();
                e.stopPropagation();
                filterToolCat(this.textContent.trim());
            });
            b._v6bound=true;
        }
    });

    // Bind tool cards - use event delegation on the tools container
    var toolsContainer=document.getElementById('tools')||document.getElementById('toolsContainer')||document.querySelector('.tools-grid');
    if(toolsContainer&&!toolsContainer._v6bound){
        toolsContainer.addEventListener('click',function(e){
            var card=e.target.closest('.tool-card');
            if(card){
                var toolId=card.getAttribute('data-name')||card.getAttribute('data-tool-id')||'';
                if(toolId)openTool(toolId);
            }
        });
        toolsContainer._v6bound=true;
    }

    // Bind login button
    var loginBtn=document.getElementById('navLoginBtn');
    if(loginBtn&&!loginBtn._v6bound){
        ...
        loginBtn._v6bound=true;
    }

    // Bind AI generate button via delegation
    document.addEventListener('click',function(e){
        if(e.target.id==='aiGenBtn'){e.preventDefault();e.stopPropagation();}
        if(e.target.id==='copyResultBtn'){e.preventDefault();copyResult();}
    });
}
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',fullInit)}
else{fullInit()}
// Delayed re-bind to catch late-rendered elements
setTimeout(fullInit,300);
setTimeout(fullInit,1000);
setTimeout(fullInit,2000);

// Polling for late-loading
var _poll=setInterval(function(){
    bindEvents();
    var changed=false;
    var k2=Object.keys(_P);
    for(var i=0;i<k2.length;i++){if(!window._AI_TEXT_TOOLS[k2[i]]){window._AI_TEXT_TOOLS[k2[i]]=_P[k2[i]];changed=true;}}
    if(changed)fullInit();
},500);
...
console.log('[fix-v6] Complete interaction layer loaded');
})();
