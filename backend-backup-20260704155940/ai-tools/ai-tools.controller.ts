...
export class AiToolsController {
  ...
  @Post('admin/tools')
  @ApiBearerAuth()
  @ApiOperation({ summary: '创建工具(管理员)' })
  async createTool(@Body() body: any) {
    const data = await this.aiToolsService.createTool(body);
    return { data };
  }
  ...
  @Put('admin/tools/:id')
  @ApiBearerAuth()
  @ApiOperation({ summary: '编辑工具(管理员)' })
  async updateTool(@Param('id') id: number, @Body() body: any) {
    const data = await this.aiToolsService.updateTool(id, body);
    return { data };
  }

  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('super_admin', 'operator')
  @Put('admin/tools/:id/status')
  @ApiBearerAuth()
  @ApiOperation({ summary: '启用/停用工具(管理员)' })
  async updateToolStatus(@Param('id') id: number, @Body('status') status: string) {
    const data = await this.aiToolsService.updateToolStatus(id, status);
    return { data };
  }
}
