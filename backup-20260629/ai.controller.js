...
let AIController = class AIController {
    ...
    async quickChat(body) {
        ...
        if (Array.isArray(body?.messages) && body.messages.length > 0) {
            ...
            return { data: { reply: "请输入你的问题。", content: "请输入你的问题。" } };
        }
        
        // 模型名称映射
        let model = body?.model || body?.modelName || "deepseek-chat";
        // 前端可能发送 deepseek-v4-pro / deepseek-v4-flash，映射到 DeepSeek 实际模型
        const modelMap = {
        ...
        try {
            const response = await fetch(baseUrl + "/chat/completions", {
                ...
                body: JSON.stringify({
                    ...
                    max_tokens: body?.maxTokens || body?.max_tokens || 4096,
                }),
            });
            if (!response.ok) {
                const errText = await response.text();
                console.error("DeepSeek API error:", response.status, errText);
                const errMsg = "AI服务调用失败(" + response.status + ")，请稍后再试。";
                return { data: { reply: errMsg, content: errMsg } };
            }
            ...
            console.error("quickChat error:", e);
            const errMsg = "AI服务异常: " + (e.message || "未知错误") + "，请稍后再试。";
            return { data: { reply: errMsg, content: errMsg } };
        }
    }
};
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({ summary: '获取所有AI Provider状态' }),
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({ summary: '获取所有可用模型列表' }),
    (0, swagger_1.ApiQuery)({ name: 'category', required: false, description: '按能力过滤: text/image/code/multimodal' }),
...
__decorate([
    ...
    (0, common_1.Post)('tools/:toolId/chat'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, swagger_1.ApiOperation)({ summary: 'AI文本对话' }),
...
__decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Post)('tools/:toolId/chat/stream'),
    (0, swagger_1.ApiBearerAuth)(),
