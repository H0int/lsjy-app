/**
 * 龙虾AI (OpenClaw) Provider
 * OpenAI 兼容 API 格式的代理服务
 * 如果 base_url 不正确，可通过环境变量 LONGXIA_BASE_URL 覆盖
 */
...
@Injectable()
export class LongxiaProvider extends BaseAIProvider {
  readonly name = 'longxia';
  readonly displayName = '龙虾AI';
  ...
  async getModels(): Promise<ModelInfo[]> {
    return [
      { id: 'gpt-4o', name: 'GPT-4o', capabilities: ['text', 'code', 'multimodal'], maxContextLength: 128000, supportStream: true },
      { id: 'gpt-4o-mini', name: 'GPT-4o Mini', capabilities: ['text', 'code', 'multimodal'], maxContextLength: 128000, supportStream: true },
      { id: 'gpt-4-turbo', name: 'GPT-4 Turbo', capabilities: ['text', 'code', 'multimodal'], maxContextLength: 128000, supportStream: true },
      { id: 'deepseek-chat', name: 'DeepSeek Chat', capabilities: ['text', 'code'], maxContextLength: 64000, supportStream: true },
    ];
  }
}
