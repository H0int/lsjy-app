...
export class SystemController {
  ...
  @Get('settings')
  @ApiBearerAuth()
  @ApiOperation({ summary: '获取系统配置(管理员)' })
  async getSettings() {
    const configs = await this.systemService.getAllConfigs();
    const settings: Record<string, Record<string, any>> = {};
    for (const c of configs) {
      if (!settings[c.configGroup]) settings[c.configGroup] = {};
      settings[c.configGroup][c.configKey] = c.configValue;
    }
  ...
  @Put('settings')
  @ApiBearerAuth()
  @ApiOperation({ summary: '保存系统配置(管理员)' })
  async saveSettings(@Body() body: Record<string, any>, @CurrentUser('id') userId: number) {
    const results: any[] = [];
    for (const [key, value] of Object.entries(body)) {
      try {
        const config = await this.systemService.updateConfig(key, String(value), userId);
        results.push(config);
      } catch { /* skip non-existing keys */ }
    }
    return { data: results };
  }
